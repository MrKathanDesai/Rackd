...
export const DeliveriesList: React.FC = () => {
  ...
  const { data: deliveries, isLoading } = useOperations('delivery', statusFilter);
  ...
  const filters = ['all', 'draft', 'waiting', 'ready', 'done', 'cancelled'];
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {canCreateDelivery && (
          <Button variant="primary" size="sm" onClick={() => navigate('/deliveries/new')}>
            + New Delivery
          </Button>
        )}
      </div>
      ...
      {data.length === 0 ? (
        <div className="border border-black p-8 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Create a delivery to dispatch roasted coffee to customers.
          </p>
        </div>
      ) : (
        <div className="border border-black">
          <Table
            columns={columns}
            data={data}
            onRowClick={(op: Operation) => navigate(`/deliveries/${op.id}`)}
            renderRow={(op: Operation) => (
              <>
                <td className="p-2 text-sm font-bold font-mono">{op.reference}</td>
                <td className="p-2 text-sm">{op.customer ?? '-'}</td>
